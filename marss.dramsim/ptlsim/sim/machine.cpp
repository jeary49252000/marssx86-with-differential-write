...
{
    static bool detailSim = false;
    static W64  start_cycles = 0;
    static W64  start_insns_committed = 0;
    static W64  update_insns = 0;
    ...
    for (;;) {
        ...
        if(total_insns_committed >= update_insns) {
            update_progress();
            // print DRAM stats
            std::string sb;
            memoryHierarchyPtr->getDramStats(sb);
            ptl_logfile << sb << endl;

            memoryHierarchyPtr->printDramStatsByMarss();

            while(update_insns <= total_insns_committed){
                update_insns += 1000000;  // find next 1M
            }
        }
        ...
        //if(detailSim == false && total_insns_committed > 200000000){
        //    detailSim = true;
        //    memoryHierarchyPtr->simulation_start();
        //    start_insns_committed = total_insns_committed;
        //    start_cycles = sim_cycle;
        //}
        ...
        if unlikely (config.stop_at_insns <= total_insns_committed ||
            ...
            //ptl_logfile << "detailed sim instructions:", (total_insns_committed - start_insns_committed), endl, "detailed sim cycles:", (sim_cycle - start_cycles);
